...
import { htmlQues } from "../../constants/question";
...
const QuizH = () => {
  ...
  if (currentQuestion === htmlQues.length) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen w-full gap-5 bg-gray-100">
        ...
        <p className="text-2xl font-bold text-gray-800">
          Your final score is {score}/{htmlQues.length}
        </p>
        <button
      ...
      <div className="w-full max-w-lg bg-white shadow-md rounded-lg p-8">
        ...
        <div className="mb-6">
          <h2 className="text-lg font-semibold text-gray-700 mb-3">
            Q{currentQuestion + 1}: {htmlQues[currentQuestion].question}
          </h2>
          <ul className="space-y-3">
            {htmlQues[currentQuestion].answerOptions.map((answer, index) => (
              <li key={index}>
                <button
